...
from utils.general_rotate import xyxy2xywh, xywh2xyxy, clean_str, xywha2xyxya, xyxya2xywha, get_rotated_coors
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if x.size > 0:
                ...
                labels[:, 1] = ratio[0] * w * labels[:, 1] + pad[0]
                labels[:, 2] = ratio[1] * h * labels[:, 2] + pad[1]
                labels[:, 3] = ratio[0] * w * labels[:, 3]
                labels[:, 4] = ratio[1] * h * labels[:, 4]
    ...
    def collate_fn(batch):
        img, label, path, shapes = zip(*batch)  # transpose
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
            # t = torch.cat([torch.ones(l.shape[0], 1) * i, l], -1)
            # l = t
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if x.size > 0:  # Normalized xywh to pixel xyxy format
            ...
            # labels[:, 1] = ratio[0] * w * labels[:, 1] + padw
            # labels[:, 2] = ratio[1] * h * labels[:, 2] + padh
            # labels[:, 3] = ratio[0] * w * labels[:, 3]
            # labels[:, 4] = ratio[1] * h * labels[:, 4]
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    n = len(targets)
    if n:
        targets[:, 5] -= a/180*math.pi

        r_limit_mask = targets[:, 5] > math.pi/2
        temp = targets[r_limit_mask].copy()
        targets[:, 3][r_limit_mask] = temp[:, 4]
        targets[:, 4][r_limit_mask] = temp[:, 3]
        targets[:, 5][r_limit_mask] -= math.pi/2

        l_limit_mask = targets[:, 5] < 0
        temp = targets[l_limit_mask].copy()
        targets[:, 3][l_limit_mask] = temp[:, 4]
        targets[:, 4][l_limit_mask] = temp[:, 3]
        targets[:, 5][l_limit_mask] += math.pi/2
...
def box_candidates(box1, box2, wh_thr=2, ar_thr=20, area_thr=0.1, eps=1e-16):  # box1(4,n), box2(4,n)
    ...
    w1, h1 = box1[2], box1[3] #box1[2] - box1[0], box1[3] - box1[1]
    w2, h2 = box2[2], box2[3] #box2[2] - box2[0], box2[3] - box2[1]
