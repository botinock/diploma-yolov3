...
from utils.general_rotate import xywh2xyxy, xyxy2xywh
...
def plot_targets_txt():  # from utils.plots import *; plot_targets_txt()
    ...
    s = ['x targets', 'y targets', 'width targets', 'height targets', 'angle targets']
    fig, ax = plt.subplots(5, 1, figsize=(8, 8), tight_layout=True)
    ax = ax.ravel()
    for i in range(5):
...
def plot_labels(labels, save_dir=Path(''), loggers=None):
    ...
    for cls, *box in labels[:1000]:
        ...
        ImageDraw.Draw(img).polygon(vertices, outline=colors[int(cls) % 10])  # plot
    ax[1].imshow(img)
    ax[1].axis('off')
